...
export class ExtractListComponent implements OnInit, AfterViewInit {
    ...
    this.subscriberService.getSubscribers()
      .subscribe({
        next: (players: PlayerEntry[]) => {
          ...
          this.dataSource = new MatTableDataSource(this.players.filter(
            (player) => player.onSite === 'yes'
          ));
  ...
  public extractSelection(): void {
    let playerToExport: PlayerEntry[];
    if (typeof this.selection.selected !== 'undefined') {
      playerToExport = this.selection.selected.filter((selectedPlayer) => selectedPlayer.onSite === 'yes');
    } else {
      playerToExport = undefined
    }
    if (typeof playerToExport !== 'undefined' && playerToExport.length > 0) {
      const dialogRef = this.dialog.open(ConfirmDownloadDialogComponent, {
        width: '450px',
        data: {
          names: this.tournamentData,
          players: playerToExport
        }
      });
          ...
          if (typeof result !== 'undefined') {
            if (typeof result.selected !== 'undefined') {
              const playersToExport: PlayerEntry[] =
                playerToExport.filter(result.exclusionPredicate.bind(dialogRef));
              ...
              this.snackBar.open('You did not select any event. Cannot export', 'Ok');
            }
          }
        }
      });
    ...
    from(players)
      .pipe(
        ...
        map((enrichedPlayer: PlayerEntry) => {
          ...
          if (enrichedPlayer.egdInfo && enrichedPlayer.level === enrichedPlayer.egdInfo.grade) {
